...
- Todos `001`, `002`, and `003` are done; next ready todo is `004`, the native visionOS point-cloud viewer.

## Working Direction

Build a native Vision Pro/visionOS viewer for the Metal-generated 3D projection volume.
...
4. Output: 3D texture/volume over `(u, v, ana)`, exported as PPM slices/contact sheet and a PLY point cloud for spatial inspection.
...
- `004` — `todos/004-ready-p1-build-native-visionos-point-cloud-viewer.md` — ready.
...
Expected: `FourDRayCoreTests` passes, and `FourDRayProbe` writes:
...
Work `todos/004-ready-p1-build-native-visionos-point-cloud-viewer.md`: create a native visionOS/RealityKit viewer or a clearly documented prototype path for the exported projection-volume point cloud.
